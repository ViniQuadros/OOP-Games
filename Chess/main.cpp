...
private:
    virtual bool AreSquaresLegal(int iSrcRow, int iScCol, int iDestRow, int iDestCol, GamePiece *GameBoard[8][8]) = 0;
    char mPieceColor;
};
...
private:
    ...
    bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece *GameBoard[8][8])
    {
        if (iSrcRow == iDestRow)
        {
            // Make sure all the squares are empty
            int iColOffset = (iDestCol - iSrcCol > 0) ? 1 : -1;
...
public:
    CBoard()
    {
        ...
        // Allocate and place the black pieces
        for (int iCol = 0; iCol < 8; ++iCol)
        {
        ...
        // Allocate and place the white pieces
        for (int iCol = 0; iCol < 8; ++iCol)
        {
    ...
    void GetNextMove(GamePiece *GameBoard[8][8])
    {
        ...
        do
        {
            ...
            cout << endl
                 ...
                 << endl
                 << endl;
            cout << "                      Keys to sysmbols used " << endl
                 << endl
                 << endl;
